...
# #flatten tensor
# def torch_flatten(a): #重塑為一維向量 (2, 3, 5)>(30,)
#     return torch.reshape(a, [-1])

# #pytorch version of np.repeat for 1D     a.shape = (3)  torch_repeat(a,4) > a.shape = (12)
# def torch_repeat(a, repeats, axis=0):
#     assert a.dim() == 1  #確保a是一維的
#     a = a.unsqueeze(dim=-1)  #加上維度在最後面
#     a = a.repeat(1, repeats)   
#     a = a.view(-1)  #使用 view()方法将矩阵重新扁平化为一维张量
#     return a


# #pytorch version of np.repeat for 2D     a.shape = (3,2)  torch_repeat(a,4) > a.shape = (4,3,2)
...
#     coords_lt = coords.floor().to(torch.int32)   #.floor 向下取值
...
#     coords_offset_lt = coords - coords_lt.float()
